...
NOTE: Operators 基于非阻塞算法实现，从而可以利用到某些调度器的工作窃取（work stealing）
特性的好处。

一些 operators 默认会使用一个指定的调度器（通常也允许开发者调整为其他调度器）例如，
...
在 Reactor 中，当你在操作链上添加 operator 的时候，你可以根据需要在 `Flux` 和 `Mono`
的实现中包装其他的 `Flux` 和 `Mono`。一旦你订阅（subscribe）了它，一个 `Subscriber` 的链
就被创建了，一直向上到第一个 publisher 。这些对开发者是不可见的，开发者所能看到的是最外一层的
`Flux` （或 `Mono`）和 `Subscription`，但是具体的任务是在中间这些跟 operator 相关的
subscriber 上处理的。

基于此，我们仔细研究一下 `publishOn` 和 `subscribeOn` 这两个 operator：

* `publishOn` 的用法和其他 operator 一样，处于订阅链（subscriber chain）中。它将下游
信号传给上游同时将上游的数据递给下游，同时执行指定的调度器 `Scheduler` 的某个工作
线程上的回调。它会 *改变后续的 operators 的执行所在线程* （知道下一个 `publishOn` 加到
这个链上。
* `subscribeOn` 用于订阅（subscription）过程，作用于那个向上的 subscriber 链（subscribe
的是才被激活，方向是向上游的）。所以，无论你把 `subscribeOn` 至于操作链（译者注：这里是
指代码表示的 operator 的链）的什么位置， *它都会影响到源头的线程执行环境（context）*。
但是，它不会影响到后续的 `publishOn`，后者仍能够切换后续 operator 的线程执行环境。

（译者加：关于 `publishOn` 和 `subscribeOn`，由于能力所限，直译的文字总是感觉不够明白，
这里补充说一下我的理解： 对于一个 operator 链， `publishOn` 从字面上看是 publish 嘛，
所以影响的是“向下的”后续的 operator 的线程执行环境，会被以后的 `publishOn` 覆盖；
而 `subscribeOn` 相反，影响的是“向上的”直到源头的各个 operator 的线程执行环境，所以
会被前边的 `subscribeOn` 覆盖。）

NOTE: 只有 operator 链中最早的 `subscribeOn` 调用才算数。

include::threading.adoc[]

[[error.handling]]
== 处理错误
TIP: 如果想了解有哪些可用于错误处理的 operator，请参考 <<which.errors,the relevant operator decision tree>>。

在响应式流中，错误（error）是终止（terminal）事件。当有错误发生时，它会导致流序列停止，
并且错误信号会沿着 operator 链条向下传递，直至遇到你定义的 `Subscriber` 及其 `onError` 方法。
...
IMPORTANT: 在你了解错误处理 operator之前，你必须牢记 *响应式流中的任何错误都是一个终止事件*。
即使用了错误处理 operator，也不会让源头流序列继续。而是将 `onError` 信号转化为一个 *新的* 序列
...
以上所有这些在 Reactor 都有相应的基于 error-handling operators 处理方式。

在开始研究这些 operator 之前，我们先准备好响应式链（reactive chain）方式和 try-catch 代码块方式（以便对比）。
...
既然我们准备了两种方式做对比，我们就来看一下不同的错误处理场景，以及相应的 operator。
...
这个 interval operator 会在每 x 单位的时间发出一个自增的 `Long` 值。
...
==== 重试
还有一个用于错误处理的 operator 你可能会用到，就是 `retry`，见文知意，用它可以对出现错误的序列进行重试。
...
就是导致出现异常并执行 retry 的那次（译者加：我在机器上测试的时候 `elapsed`
...
来判断对某次错误是否要重试。这个伴随 `Flux` 是由 operator 创建的，但是由开发者包装它，
从而实现对重试操作的配置。
...
=== 在 Operator 或 Function 中处理异常
总体来说，所有的 operators 自身都可能包含触发异常的代码，或自定义的可能导致失败的代码，
所以它们都自带一些错误处理方式。
...
可用于这种方式（我们马上会讲到）。
3. 如果你气我返回一个 `Flux` （例如在 `flatMap` 中），将异常包装在一个产生错误的 `Flux`中：
