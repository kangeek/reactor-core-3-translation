...
无论你是编写了一个简单的 Reactor 操作链，还是开发了自定义的操作符，对它进行
自动化的测试总是一个好主意。
...
`StepVerifier` 可以用来测试基于时间的操作符，从而避免测试的长时间运行。可以使用构造器
...
调度器。这些基于时间的操作符通常默认使用 `Schedulers.parallel()` 调度器。（虚拟时间的）
...
`StepVerifier` 有一些期望方法可以用来测试 `Context`：
  ...
  被测试的 `Publisher` 并不是一个响应式的，或没有任何操作符能够传递 `Context`
...
考虑下边这个方法，它构建了一条操作链，并使用 `switchIfEmpty` 方法在源为空的情况下，
替换成另一个源。
